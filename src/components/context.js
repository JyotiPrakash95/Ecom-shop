...
async function placeOrder() {
  try {
    // const { setIsLoading, setCartItems, setCartTotal, auth } = useValue(); // Adjust this based on your context
    // setIsLoading(true);

    const userId = auth.currentUser.uid;
    console.log(auth.currentUser.uid) // current UserID
    const cartRef = ref(database, `usersCarts/${userId}/myCart`);
    const cartTotalRef = ref(database, `usersCarts/${userId}/cartTotal`);
    ...
    const order = {
      ...
      total: cartTotal, // Make sure `cartTotal` is defined
    };

    // Push the order to the "myOrders" node in the database
    const orderKey = push(ordersRef).key; // Get the key generated by push
    await set(ref(ordersRef, orderKey), order);
